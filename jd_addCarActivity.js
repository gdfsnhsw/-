            ...
            if($.taskList){
                for(let j =0;j < $.taskList.length;j++){
                    ...
                    if(!$.isContinue){
                        ...
                        await findSkus(item)
                        if($.skus.length > 0){
                            console.log(`\n******第${j+1}个活动,店铺名:${item.shopName},开始加购店铺*********\n`)
                            await followShop(item);
                            console.log(`\n******第${j+1}个活动,店铺名:${item.shopName},需要加购${$.skus.length}个商品*********\n`)
                            ...
                            console.log(`\n******第${j+1}个活动,店铺名:${item.shopName},没有需要加购的商品，跳过*********\n`)
                        }
    ...
    return new Promise(resolve => {
        let options = {
            ...
            headers: {
                ...
                'Referer':`https://lzkj-isv.isvjd.com/wxCollectionActivity/activity2/${item.activityId}?activityId=${item.activityId}&adsource=cjhdc`,
            ...
            try {
                if (err) {
                    ...
                    if(data){
                        ...
                        if(data.data && data.data == "draw_error"){
...
function followShop(item) {
    return new Promise(resolve => {
        let options = {
            ...
            body: `userId=${item.venderId}&buyerNick=${encodeURIComponent($.secretPin)}&activityId=${item.activityId}&activityType=6`,
            headers: {
                'Accept':'application/json, text/javascript, */*; q=0.01',
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8',
...
function findSkus(item) {
    return new Promise(resolve => {
        let options = {
            ...
            body: `actId=${item.activityId}&userId=${$.userId}&type=6`,
            headers: {
                'Accept':'application/json, text/javascript, */*; q=0.01',
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8',
...
function queryActivityInfo(item) {
    return new Promise(resolve => {
        let options = {
            url: `https://lzkj-isv.isvjd.com/wxCollectionActivity/activityContent`,
            body: `activityId=${item.activityId}&pin=${encodeURIComponent($.secretPin)}`,
            headers: {
                'Accept':'application/json, text/javascript, */*; q=0.01',
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                ...
                if (err) {
                    ...
                    if(data && data != ""){
                        ...
                        if(data.data){
                            ...
                            let drawInfo = data.data.drawInfo
                            if(drawInfo && (drawInfo.drawInfoType == 6 || drawInfo.drawInfoType == 16)){
                            }else{
                                $.isContinue = true
                            }
                        }
...
function getAllActivitys() {
    return new Promise(resolve => {
        let options = {
            ...
            body: `pin=${encodeURIComponent($.secretPin)}&aggrateActType=7&topNewType=1&pageNo=1&pageSize=200`,
            headers: {
                'Accept':'application/json, text/javascript, */*; q=0.01',
                'User-Agent': `Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1`,
                'Content-Type':'application/x-www-form-urlencoded; charset=UTF-8',
                ...
                'Host':'lzkjdz-isv.isvjd.com',
                'Origin':'https://lzkjdz-isv.isvjd.com',
                'Referer':'https://lzkjdz-isv.isvjd.com/wxAssemblePage/moreActivity?activityId=67dfd244aacb438893a73a03785a48c7&type=7',
                'Cookie': `LZ_TOKEN_KEY=${$.LZ_TOKEN_KEY}; LZ_TOKEN_VALUE=${$.LZ_TOKEN_VALUE};lz_wq_auth_token=${$.isvObfuscatorToken}`,
            }
        }
        $.post(options, async (err, resp, data) => {
