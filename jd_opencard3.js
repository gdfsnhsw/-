...
!(async () => {
    ...
    $.shareUuid='58aac9ee96214c7ab6d0cccc5e720d4a'
    $.activityId = 'dz2108100000190101'
    console.log(`入口:\nhttps://lzdz1-isv.isvjcloud.com/dingzhi/jq/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
...
async function run(){
    try{
        ...
        $.log("加购: " + $.addSku)
        if(!$.addSku) await saveTask('加购', 2, 2);
...
function saveTask(title, taskType, taskValue) {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/jq/openCard/saveTask', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.toStr(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function shopactivityId(functionId) {
    return {
        ...
        headers: {
            ...
            'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz1-isv.isvjcloud.com/dingzhi/jq/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            'Cookie': cookie
        }
    }
}
...
function checkOpenCard() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/jq/openCard/checkOpenCard', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.toStr(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function getActorUuid() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/jq/openCard/activityContent',body), async(err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.toStr(err)}`)
                    console.log(`${$.name} API请求失败，请检查网路重试`)
...
function accessLogWithAD() {
    return new Promise(resolve => {
        let pageurl = `https://lzdz1-isv.isvjcloud.com/dingzhi/jq/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
    ...
    return new Promise(resolve => {
        let get = {
            ...
            headers: {
                ...
                "Referer":`https://lzdz1-isv.isvjcloud.com/dingzhi/jq/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
                "User-Agent": $.UA,
            }
        }
...
function getCk() {
    return new Promise(resolve => {
        let get = {
            url:`https://lzdz1-isv.isvjcloud.com/dingzhi/jq/openCard/activity/?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            headers: {
                "Cookie": cookie,
                "User-Agent": $.UA,
            }
