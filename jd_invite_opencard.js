...
async function main() {
    ...
    let _0x588e1c = await getInviteRecord();
    let _0x505334 = await getIsInvited();
    await adLog();
    let _0x193ff7 = await getAcceptInvite();
    for (let activityInfo of activityInfos.shopUrl) {
        $.log('入会 ' + activityInfo);
        await join(activityInfo);
    }
    await join2();
...
function getActivityInfo() {
    return new Promise(resolve => {
        let body = 'activityId=619a39c4eb26417aaa0b2b0d921a8dc4';
...
function getInviteRecord() {
    return new Promise(resolve => {
        let body = 'activityId=619a39c4eb26417aaa0b2b0d921a8dc4&inviter=' + encodeURIComponent($.myPingData.secretPin) + '&pageNo=1&pageSize=15&type=0';
        $.post(taskPostUrl('/microDz/invite/activity/wx/inviteRecord', body, 'https://cjhydz-isv.isvjcloud.com/microDz/invite/activity/wx/inviteRecord'),
            async (err, resp, data) => {
            try {
                if (err) {
                    console.log($.name + ' API请求失败，请检查网路重试');
...
function getIsInvited() {
    return new Promise(resolve => {
        let body = 'activityId=619a39c4eb26417aaa0b2b0d921a8dc4&pin=' + encodeURIComponent($.myPingData.secretPin);
...
function getAcceptInvite() {
    return new Promise(resolve => {
        let body = 'activityId=619a39c4eb26417aaa0b2b0d921a8dc4&inviter=' + encodeURIComponent($.DQCK) + '&inviterImg=http%3A%2F%2Fstorage.360buyimg.com%2Fi.imageUpload%2F6a645f73495a76594b617266594d5731363237363637373836333031_mid.jpg&inviterNick=' + encodeURIComponent($.inviterNick) + '&invitee=' + encodeURIComponent($.myPingData.secretPin) + '&inviteeImg=https%3A%2F%2Fimg10.360buyimg.com%2Fimgzone%2Fjfs%2Ft1%2F21383%2F2%2F6633%2F3879%2F5c5138d8E0967ccf2%2F91da57c5e2166005.jpg&inviteeNick=' + $.UserName;
        $.post(taskPostUrl('/microDz/invite/activity/wx/acceptInvite', body, 'https://cjhydz-isv.isvjcloud.com/microDz/invite/activity/wx/acceptInvite'),
            async (err, resp, data) => {
            try {
                if (err) {
                    console.log($.name + ' API请求失败，请检查网路重试');
...
function getOpenCardAllStatuesNew() {
    return new Promise(resolve => {
        let body = 'activityId=619a39c4eb26417aaa0b2b0d921a8dc4&pin=' + encodeURIComponent($.myPingData.secretPin) + '&isInvited=1';
...
function ruhui2() {
    return {
        ...
        'headers': {
            ...
            'User-Agent': $.UA,
            'content-type': 'application/x-www-form-urlencoded',
            'Referer': 'https://shopmember.m.jd.com/shopcard/?venderId=10314962&shopId=10175519&channel=8802&returnUrl=https://cjhydz-isv.isvjcloud.com/microDz/invite/activity/wx/view/index/4438790?activityId=619a39c4eb26417aaa0b2b0d921a8dc4&inviter=' + $.DQCK + '&inviterImg=http://storage.360buyimg.com/i.imageUpload/6a645f3639613631633262633231303231363238313235383234363133_mid.jpg&inviterNickName=Tsukasa%E9%B1%BC%E8%9B%8B&shareuserid4minipg=DQCK%2FksVMxxhAtP2wbQfI9A1Drq3za4lh6LFLfledF1cdSiqMbCx5edEEaL3RnCSkdK3rLBQpEQH9V4tdrrh0w%3D%3D&shopid=599119&lng=113.388014&lat=22.510994&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0',
            'Cookie': cookie
        }
    };
}
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            'url': 'https://cjhydz-isv.isvjcloud.com/microDz/invite/activity/wx/view/index/4438790?activityId=619a39c4eb26417aaa0b2b0d921a8dc4&inviter=' + $.inviter + '&inviterImg=http://storage.360buyimg.com/i.imageUpload/6a645f3639613631633262633231303231363238313235383234363133_mid.jpg&inviterNickName=Tsukasa%E9%B1%BC%E8%9B%8B&shareuserid4minipg=DQCK%2FksVMxxhAtP2wbQfI9A1Drq3za4lh6LFLfledF1cdSiqMbCx5edEEaL3RnCSkdK3rLBQpEQH9V4tdrrh0w%3D%3D&shopid=599119&lng=113.388014&lat=22.510994&sid=09fdc8e908526b5538a4ad4a265f40dw&un_area=19_1657_52093_0',
            'headers': {
                'User-Agent': $.UA,
                'Host': 'cjhydz-isv.isvjcloud.com',
...
function getSimpleActInfoVo() {
    return new Promise(resolve => {
        $.post(taskPostUrl('/dz/common/getSimpleActInfoVo', 'activityId=c225ad5922cf4ac8b4a68fd37f486088', 'https://lzdz1-isv.isvjcloud.com/dz/common/getSimpleActInfoVo'),
            async (err, resp, data) => {
            try {
                if (err) {
                    console.log('' + JSON.stringify(err));
...
function adLog() {
    return new Promise(resolve => {
        $.post({
            ...
            'body': 'venderId=0&code=99&pin=' + encodeURIComponent( $.myPingData.secretPin) + '&activityId=619a39c4eb26417aaa0b2b0d921a8dc4&pageUrl=https%3A%2F%2Fcjhydz-isv.isvjcloud.com%2FmicroDz%2Finvite%2Factivity%2Fwx%2Fview%2Findex%2F2388747%3FactivityId%3D619a39c4eb26417aaa0b2b0d921a8dc4%26inviter%3Djzv2jbYRftpJUlB6E7%2Ff3%2FL7ldxmgdCpzmNX2HGi4eBuw30v%2FPoVBgxrRDHHbTlt%26inviterImg%3Dhttp%3A%2F%2Fstorage.360buyimg.com%2Fi.imageUpload%2F6a645f73495a76594b617266594d5731363237363637373836333031%5Fmid.jpg%26inviterNickName%3D%E4%B8%8A%E8%AF%BE%E6%97%A0%E8%81%8A%E5%90%97%26shareuserid4minipg%3D4oSXfUlJ1qzTqmn3%252Fgy2c9A1Drq3za4lh6LFLfledF1cdSiqMbCx5edEEaL3RnCSkdK3rLBQpEQH9V4tdrrh0w%253D%253D%26shopid%3D599119%26lng%3D0.000000%26lat%3D0.000000%26sid%3D5fa6c7778669e4865e2e7e7ba5ea098w%26un%5Farea%3D17%5F1458%5F1463%5F43894&subType=',
            'headers': {
                'User-Agent': 'Mozilla/5.0 (Linux; U; Android 8.0.0; zh-cn; Mi Note 2 Build/OPR1.170623.032) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/61.0.3163.128 Mobile Safari/537.36 XiaoMi/MiuiBrowser/10.1.1',
                'Host': 'cjhydz-isv.isvjcloud.com',
                'Content-Type': 'application/x-www-form-urlencoded; Charset=UTF-8',
