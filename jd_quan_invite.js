...
没有适配加购变量 所以是不加购
————————————————
入口：[8.11-8.15 星动七夕 纵享丝滑 (https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=375dbaa9e32c4c70bb8357836956ed2e&shareUuid=d87a80e864dd45909d11f098b9efb3d0)]
============Quantumultx===============
[task_local]
#8.11-8.15 星动七夕 纵享丝滑
...
!(async () => {
    ...
    $.shareUuid = 'd316bda04d0b4ce0b69def8c4c86e405'
    $.activityId = 'dz2009100009970144'
    console.log(`入口:\nhttps://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=${$.activityId}&shareUuid=${$.shareUuid}`)
...
function addSku() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/biouquan/saveTask', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function followShop() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/biouquan/followShop', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function shopactivityId(functionId) {
    return {
        ...
        headers: {
            ...
            'Referer': `https://shopmember.m.jd.com/shopcard/?venderId=${functionId}&shopId=${functionId}&venderType=5&channel=401&returnUrl=https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            'Cookie': cookie
        }
    }
}
...
function startDraw(type) {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/biouquan/startDraw', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
...
function checkOpenCard() {
    return new Promise(resolve => {
        ...
        $.post(taskPostUrl('/dingzhi/biouquan/checkOpenCard', body), async (err, resp, data) => {
            try {
                if (err) {
                    console.log(`${$.name} API请求失败，请检查网路重试`)
                } else {
    ...
    return new Promise(resolve => {
        $.post({
            ...
            headers: {
                ...
                'Referer':`https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            ...
            try {
                if (err) {
                    ...
                    if(typeof res == 'object' && res.result === true){
                        ...
                        if(typeof res.data.userId != 'undefined') $.userId = res.data.userId
...
function adLog() {
    return new Promise(resolve => {
        let pageurl = `https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=${$.activityId}&shareUuid=${$.shareUuid}`
        $.post({
...
function getHtml() {
    return new Promise(resolve => {
        $.get({
            url: `https://lzdz-isv.isvjcloud.com/dingzhi/biouquan/activity/1760960?activityId=${$.activityId}&shareUuid=${$.shareUuid}`,
            headers: {
                'User-Agent': $.UA,
                'Host':'lzdz-isv.isvjcloud.com',
